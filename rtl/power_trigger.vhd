...
assing_power_o: for bm in 0 to num_beams-1 generate
	avg_power(bm)<=unsigned(power_integration_o(2*18*(bm+1)-18-1 downto 2*18*bm));
	avg_power_overlap(bm)<=unsigned(power_integration_o(2*18*(bm+1)-1 downto 2*18*bm+18));
end generate;
...
--assign_beaming_i: for ch in 0 to 3 generate
--	assign_sams_i: for sam in 0 to 15 generate
--		beaming_i(8*16*ch+8*(sam+1)-1 downto 8*16*ch+8*i)<=std_logic_vector(interp_data(ch,sam));
--	end generate;
--end generate;
--assing_beaming_o: for bm in 0 to num_beams generate
--	assign_sams_o: for sam in 0 to 15 generate
--		phased_beam_waves(bm,sam)<=signed(beam_data_o(bm*8*16+8*(sam+1)-1 downto bm*8*16+8*sam));
--	end generate;
--end generate;