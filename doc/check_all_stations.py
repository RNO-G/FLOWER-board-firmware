...
all_delays=np.zeros((7,4))
all_depths=np.zeros((7,4))
...
for i_stat,station in enumerate(stations):
...
for i in range(num_beams):
    ax[i].scatter(np.linspace(0,.8,len(all_lookbacks[:,0,i])),all_lookbacks[:,0,i]-round(np.mean(all_lookbacks[:,0,i])))
    ax[i].scatter(1+np.linspace(0,.8,len(all_lookbacks[:,0,i])),all_lookbacks[:,1,i]-round(np.mean(all_lookbacks[:,1,i])))
    ax[i].scatter(2+np.linspace(0,.8,len(all_lookbacks[:,0,i])),all_lookbacks[:,2,i]-round(np.mean(all_lookbacks[:,2,i])))
    ax[i].scatter(3+np.linspace(0,.8,len(all_lookbacks[:,0,i])),all_lookbacks[:,3,i]-round(np.mean(all_lookbacks[:,3,i])))
    ax[i].set_ylabel('beam %i'%i)
    ax[i].set_xticks([0,1,2,3])
fig.suptitle('Beam delays from mean')
